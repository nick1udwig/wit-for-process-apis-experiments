...
fn init(our: Address) {
    ...
    let Some((name, who)) = args.split_once(" ") else {
        println!("usage: {}@download:ft_composed:template.os file_name who", our.node());
        return
    };
    let our: Address = format!("{}@ft_composed:ft_composed:template.os", our.node())
        .parse()
        .unwrap();

    let target: Address = format!("{}@ft_composed:ft_composed:template.os", who)
        .parse()
        .unwrap();
